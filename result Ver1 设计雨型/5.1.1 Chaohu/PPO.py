...
class PPO:
    def __init__(self, env, ep, batch, num_rain, t='ppo2',AFI=True):
        self.t = t
        self.ep = ep
...
#               + tf.reduce_sum(tf.log(std), axis=-1)
    ...
    def train(self):
        ...
        if self.AFI:
            saver.restore(self.sess,"./save/"+self.t+"_AFI_model.ckpt")
        else:
            saver.restore(self.sess,"./save/"+self.t+"model.ckpt")
        ...
        for j in range(self.ep):
            ...
            #保存模型
            if self.AFI: 
                sp=saver.save(self.sess,"./save/"+self.t+"_AFI_model.ckpt")
            else:
                sp=saver.save(self.sess,"./save/"+self.t+"model.ckpt")
