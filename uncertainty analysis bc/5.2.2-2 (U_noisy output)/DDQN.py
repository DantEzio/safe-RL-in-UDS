    ...
    def test_inp(self,test_num,rd,rainid):
        ...
        for testid in range(test_num):
            ...
            while True:
                ...
                a = self.action(observation)
                X=np.random.uniform(0.95,1.05,size=a.shape)
                action = (self.action_table[a,:]*X*rd).tolist()
                
                a = self.action(observation)
                X=np.random.normal(rd/10,0.01,size=a.shape)
                action = (self.action_table[a,:]+0.1*X).tolist()
