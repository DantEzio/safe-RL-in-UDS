...
#        self.rain=change_rain.gen_rain(self.date_t[-1],A,C,P,b,n,R,self.deltt)
        ...
        for rg in ['RG1','RG2','RG3','RG4']:
            change_rain.change_rain(raindata[rg],self.orf_rain+'.inp',rg)#先修改self.orf_rain，再复制给staf
        ...
        rain_sum=0
        for rg in ['RG1','RG2','RG3','RG4']:
            rain_sum+=np.sum(raindata[rg][self.date_t[self.iten]:self.date_t[self.iten+1]])/np.max(raindata[rg])
    ...
    def step(self,a,raindata):
        ...
        if self.safety:
            ...
            self.upper_num=0#*self.iten/60#10分钟时间步长，不超过两次启停，总共不超过2*iten次
            self.upper_R=0.5
            ...
            c1=np.mean(np.array(N_wl)-0.9*5)
            c2=np.mean(np.array(num)-self.upper_num)
            #c3=Sev-self.upper_R
