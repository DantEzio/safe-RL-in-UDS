...
class PPO:
    def __init__(self, env, ep, batch, num_rain, t='ppo1'):
        ...
        self.bound_high = 32#self.env.action_space[0]
        self.bound_low = 1#self.env.action_space[1]
        ...
        self.rainData=np.loadtxt('./sim/trainRainFile.txt',delimiter=',')/3#读取训练降雨数据
        self.testRainData=np.loadtxt('./sim/testRainFile.txt',delimiter=',')/3#读取测试降雨数据
    ...
    def _build_actor(self, name, trainable):
        ...
        with tf.compat.v1.variable_scope(name,reuse=tf.compat.v1.AUTO_REUSE):
            ...
            mu = (self.bound_high-self.bound_low) * tf.layers.dense(x, 1, tf.nn.tanh, trainable=trainable,
    ...
    def build_model(self):
        ...
        self.action = tf.compat.v1.placeholder(tf.float32, [None, 1], 'action')
    ...
    def choose_action(self, state):
        ...
        action=np.int(action*self.action_size)
