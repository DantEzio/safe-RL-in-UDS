...
class DDQN:
    def __init__(self,step=200,batch_size=24,num_rain=10,env=gym.make('Pendulum-v0').unwrapped,t='dqn'):
        ...
        #self.testRainData=np.loadtxt('./sim/real_rain_data.txt',delimiter=' ')#读取真实降雨数据
        #add white noise
    ...
    def test_inp(self,test_num,rd,rainid,testdataid):
        ...
        for testid in range(test_num):
            ...
            while True:
                ...
                #uncertainty
                X=np.random.uniform(1-rd/10,1+rd/10,size=observation.shape)
                #observation=observation*X
                observation=observation*X
                
                a = self.action(observation)
                
                X=np.random.uniform(1-rd/10,1+rd/10,size=a.shape)
                action = (self.action_table[a,:]*X).tolist()
            ...
            #save RLC .inp and .rpt
            #sout='./'+savefile+'/'+self.t+'_test_result/'+str(testid)+'_'+str(rainid)+'.rpt'
            #sin=self.env.staf+'.rpt'
            #self.env.copy_result(sout,sin)
            #sout='./'+savefile+'/'+self.t+'_test_result/'+str(testid)+'_'+str(rainid)+'.inp'
            #sin=self.env.staf+'.inp'
            #self.env.copy_result(sout,sin)
            #self.env.copy_result(sout,sin)
        #保存所有降雨的flooding过程线
        df = pd.DataFrame(np.array(flooding_logs).T)
        df.to_csv('./'+savefile+'/'+self.t+'_test_result/Rain'+str(rainid)+'_'+'randomlevel'+str(rd)+'_'+self.t+'flooding_vs_t.csv', index=False, encoding='utf-8')
